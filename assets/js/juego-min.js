const miModulo = (() => {
    ...
    let h = document.querySelectorAll(".divCartas"),
        ...
        j = () => {
            d = [];
            for (let a = 2; a <= 10; a++)
                for (let b of e) d.push(a + b);
            for (let c of e)
                for (let g of f) d.push(g + c);
            return _.shuffle(d)
        },
        ...
        p = a => {
            ...
            do {
                let c = k();
                b = m(c, g.length - 1), n(c, g.length - 1)
            } while (b < a && a <= 21) o()
        };
    return a.addEventListener("click", () => {
        let d = k(),
            c = m(d, 0);
        n(d, 0), c > 21 ? (console.warn("Lo siento mucho, perdiste"), a.disabled = !0, b.disabled = !0, p(c)) : 21 === c && (console.warn("21, genial!"), a.disabled = !0, b.disabled = !0, p(c))
    }), b.addEventListener("click", () => { a.disabled = !0, b.disabled = !0, p(g[0]) }), { nuevoJuego: c }
})()